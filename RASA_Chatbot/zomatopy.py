...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", start=0, count=20, sort="rating", order="desc"):
        """
        Takes either query, latitude and longitude or cuisine as input.
        Returns a list of Restaurant IDs.
        """
        ...
        r = (requests.get(base_url + "search?q=" + str(query) +  "&start=" + str(start) + "&count=" + str(count) + "&lat=" + str(latitude) + \
            "&lon=" + str(longitude) + "&cuisines=" + str(cuisines) + "&sort=" + sort + "&order=" + order, headers=headers).content).decode("utf-8")
        return r#a = ast.literal_eval(r)
